...
app.get('/ConsultaStockPorLote', async (req, res) => {
  try {
    // Realizar la solicitud al Service Layer de SAP para obtener todos los registros de la consulta SQL registrada
    const response = await sl.get(`SQLQueries('ConsultaStockPorLote')/List`);
...
app.get('/ConsultaStockPorLote/:itemCode', async (req, res) => {
  try {
    const itemCode = req.params.itemCode;

    // Realizar la solicitud al Service Layer de SAP usando POST con el parámetro ItemCode
    const response = await sl.post(`SQLQueries('ConsultaStockPorLoteConParametro')/List`, {
      Parameters: [
        {
          Name: 'ItemCode',
          Value: itemCode
